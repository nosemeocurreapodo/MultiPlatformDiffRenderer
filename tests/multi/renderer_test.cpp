...
TEST_F(DataLoader, TestRenderDepth)
{
    int src = 0;
    int dst = 50;
    ...
    Mesh mesh = MeshCPU(vertices, texcoords, weights);
    Renderer renderer = DepthRendererCPU();
    Texture<ImageType> image = TextureCPU<ImageType>(w, h, 1, 0);
    Texture<float> depth = TextureCPU<float>(w, h, 1, -1);
...
TEST_F(DataLoader, TestRenderImage)
{
    int src = 0;
    int dst = 50;
    ...
    Renderer renderer = ImageRendererCPU();
    Texture<ImageType> image = TextureCPU<ImageType>(w, h, 1, 0);
    Texture<ImageType> output = TextureCPU<ImageType>(w, h, 1, -1);
    image.FromCPU((ImageType *)image_src_CV.data);
