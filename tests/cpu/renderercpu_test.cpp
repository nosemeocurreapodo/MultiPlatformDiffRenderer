...
TEST_F(DataLoader, TestJPoseRendererCPU)
{
    ...
    MeshCPU mesh(vertices, texcoords, weights);
    JPoseRendererCPU renderer;
    TextureCPU<cpu::ImageType> image(w, h, 1, 0);
    TextureCPU<cpu::Vec6> output(w, h, 1, -1);
    image.FromCPU((cpu::ImageType *)image_src_CV.data);
    ...
    output.ToCPU((cpu::Vec6 *)output_data.data);

    cv::Mat output_imageCV = cv::Mat(h, w, GetOpenCVFormat(GetTypeIndex<cpu::Vec3>(), 1));
    ...
    for(int i = 0; i < h; ++i)
    {
        for(int j = 0; j < w; ++j)
        {
            ...
            output_imageCV.at<cpu::Vec3>(i, j) = cpu::Vec3(out(0), out(1), out(2));
        }
    }
