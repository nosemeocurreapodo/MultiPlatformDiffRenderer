...
TEST_F(RendererBenchmark, BenchmarkDepthRendererCPU)
{
    ...
    {
        timer_.Start();
        renderer.Render(mesh, pose_transform, cam_, out_lvl, output);
...
TEST_F(RendererBenchmark, MemoryUsageBenchmark)
{
    ...
    for (int res : resolutions)
    {
        ...
        timer_.Start();
        {
            ...
            DepthRendererCPU renderer_cpu;
            renderer_cpu.Render(mesh_cpu, SE3(), cam_, 0, output_cpu);
        }
        ...
        timer_.Start();
        {
            ...
            DepthRendererGL renderer_gl;
            renderer_gl.Render(mesh_gl, SE3(), cam_, 0, output_gl);
            glFinish();
        }
