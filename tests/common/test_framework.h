...
protected:
    ...
    template<typename T>
    void UploadMatToTextureCPU(TextureCPU<T>& tex, int lvl, const cv::Mat& mat) {
        auto mapped = tex.MapWrite(lvl);
        std::memcpy(mapped.data(), mat.ptr(), mat.total() * sizeof(T));
        tex.generate_mipmaps(0);
    }
    
    template<typename T>
    cv::Mat DownloadTextureFromCPU(const TextureCPU<T>& tex, int lvl, int w, int h, int cv_type) {
        cv::Mat result(h, w, cv_type);
        auto mapped = tex.MapRead(lvl);
        std::memcpy(result.ptr(), mapped.data(), w * h * sizeof(T));
        return result;
    }
    
#ifdef COMPILE_GL
    template<typename T>
    void UploadMatToTextureGL(TextureGL<T>& texture, int level, const cv::Mat& mat) {
        auto mapped = texture.MapWrite(level);
        
        if constexpr (std::is_same_v<T, float>) {
            cv::Mat mat_f;
            mat.convertTo(mat_f, CV_32F);
            std::memcpy(mapped.data(), mat_f.data, mat_f.total() * sizeof(float));
        } else if constexpr (std::is_same_v<T, Scalar>) {
            cv::Mat mat_f;
            mat.convertTo(mat_f, CV_32F);
            std::memcpy(mapped.data(), mat_f.data, mat_f.total() * sizeof(float));
        }
        // Note: Vec3 and Vec4 types are not trivially copyable, so we avoid them in GL uploads
    }
    
    template<typename T>
    cv::Mat DownloadTextureFromGL(const TextureGL<T>& tex, int lvl, int w, int h, int cv_type) {
        cv::Mat result(h, w, cv_type);
        auto mapped = tex.MapRead(lvl);
        std::memcpy(result.ptr(), mapped.data(), w * h * sizeof(T));
        return result;
