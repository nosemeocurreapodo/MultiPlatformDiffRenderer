...
public:
    ...
    */
          ...
          wei_buffer_(weights)
    {
        ...
        {
            // Delaunay returns unsigned int; convert to index_type
    ...
    // Counts
    size_type vertex_count() const noexcept { return pos_buffer_.size() / 3; }
    size_type index_count() const noexcept { return ebo_buffer_.size(); }
    size_type triangle_count() const noexcept { return index_count() / 3; }
    ...
    void rebuild_indices_from_uv()
    {
        ...
        auto s = tex_buffer_.size();
        std::vector<unsigned int> tris = BuildTriangles(
            std::vector<float>(m[0], m[s-1]));
...
private:
    void validate_() const
    {
        ...
        const size_type nverts = pos_buffer_.size() / 3;
        ...
        {
            assert(idx[i] < static_cast<index_type>(nverts));
        }
#endif
    }
