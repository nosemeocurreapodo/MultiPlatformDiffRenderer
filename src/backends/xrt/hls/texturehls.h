...
public:
    ...
    TextureBRAM(T nodata)
        : nodata_(nodata)
    {
    }

    TextureBRAM(T nodata, T *base)
        : TextureBRAM(nodata)
    {
        for (int i = 0; i < W * H; i++)
        {
#pragma HLS loop_tripcount min = 307200 max = 307200 avg = 307200
    ...
    // Introspection
    unsigned int width(unsigned int lvl) const { return W; }
    unsigned int height(unsigned int lvl) const { return H; }
    // unsigned int levels() const { return n_levels_; }
