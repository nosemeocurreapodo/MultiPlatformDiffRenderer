...
public:
    ...
    // move-only RAII
    BaseRendererXRT(BaseRendererXRT &&o) noexcept { *this = std::move(o); }
    BaseRendererXRT &operator=(BaseRendererXRT &&o) noexcept
    {
        if (this != &o)
        {
            destroy_();
    ...
    BaseRendererXRT(const BaseRendererXRT &) = delete;
    BaseRendererXRT &operator=(const BaseRendererXRT &) = delete;

    virtual ~BaseRendererXRT() { destroy_(); }
...
class DepthRendererXRT : public BaseRendererXRT
{
public:
    DepthRendererXRT() : DepthRendererXRT()
    {
    ...
    {
        assert(kernel_.group_id(0) == mesh.pos.get_memory_group());
        assert(kernel_.group_id(1) == mesh.pos.get_memory_group());
        assert(kernel_.group_id(2) == mesh.pos.get_memory_group());
        assert(kernel_.group_id(3) == mesh.ebo.get_memory_group());
        assert(kernel_.group_id(4) == depth_texture.storage_.get_memory_group());
