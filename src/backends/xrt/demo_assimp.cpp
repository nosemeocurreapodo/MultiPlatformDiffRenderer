...
static bool LoadAssimpMesh(const std::string& path,
    ...
    vertices.clear(); texcoords.clear(); weights.clear(); indices.clear();
    ...
    if (outCenter || outRadius) {
        ...
        if (outCenter) { outCenter->x = c.x(); outCenter->y = c.y(); outCenter->z = c.z(); }
        if (outRadius) {
            float r = (maxB - c).norm();
            *outRadius = r;
        }
...
static cv::Mat TryLoadDiffuseTexture(const std::string& modelPath) {
    ...
    if (!scene || !scene->HasMaterials()) return cv::Mat();
...
static Camera MakeCamera(unsigned w, unsigned h, float fov_deg = 60.0f) {
...
static SE3 MakePose(const Eigen::Matrix3f& R, const Eigen::Vector3f& t) {
    return SE3(R, t);
}
...
int main(int argc, char** argv)
{
    ...
    if (argc < 4 || argc > 5) {
        std::cout << "please provide: xclbin_file device_id model_path [texture_override_path]" << std::endl;
        return 1;
    }
    ...
    MeshXRT mesh(vertices, texcoords, weights, indices,
                 ...
                 renderer.kernel_.group_id(2), renderer.kernel_.group_id(3));
    ...
    std::vector<double> times; times.reserve(max_frames);
    ...
    for (int i = 0; i < max_frames; ++i) {
        ...
        Eigen::Matrix3f R =
            (Eigen::AngleAxisf(0.15f * t, Eigen::Vector3f::UnitX()) *
             Eigen::AngleAxisf(0.6f  * t, Eigen::Vector3f::UnitY())).toRotationMatrix();
        ...
        SE3 pose_transform = MakePose(Rc, tc);
        ...
        cv::imshow("RasterizerXRT", out_color);
        if (i % 60 == 0) {
    ...
    std::cout << "XRT Rasterizer Performance:\n";
