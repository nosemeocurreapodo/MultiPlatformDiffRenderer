...
int main(int argc, char **argv)
{
    ...
    TextureXRT<float> output_image(w, h, -1.0f, renderer.kernel_.group_id(5));
    TextureXRT<float> output_depth(w, h, -1.0f, renderer.kernel_.group_id(6));
    TextureXRT<linalg::Vec3<float>> output_jtra(w, h, linalg::Vec3<float>(0.0f, 0.0f, 0.0f), renderer.kernel_.group_id(7));
    TextureXRT<linalg::Vec3<float>> output_jrot(w, h, linalg::Vec3<float>(0.0f, 0.0f, 0.0f), renderer.kernel_.group_id(8));
    TextureXRT<linalg::Vec3<float>> output_pids(w, h, linalg::Vec3<float>(-1.0f, -1.0f, -1.0f), renderer.kernel_.group_id(9));
    ...
    for (int i = 0; i < iterations; ++i)
    {
        ...
        renderer.Render(mesh, pose_transform, cam, in_lvl, out_lvl, input, output_image, output_depth, output_jtra, output_jrot, output_pids);
        auto t1 = std::chrono::high_resolution_clock::now();
        double time_ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
        times.push_back(time_ms);

        cv::Mat image_out_cv = DownloadTextureToMat(output_image, out_lvl, CV_32FC1);
        cv::Mat depth_out_cv = DownloadTextureToMat(output_depth, out_lvl, CV_32FC1);
        cv::Mat jtra_out_cv = DownloadTextureToMat(output_jtra, out_lvl, CV_32FC3);
        cv::Mat jrot_out_cv = DownloadTextureToMat(output_jrot, out_lvl, CV_32FC3);
        cv::Mat pids_out_cv = DownloadTextureToMat(output_pids, out_lvl, CV_32FC3);

        // double depthError = ComputeImageError<float>(depth_dst_CV, output_depthCV, -1.0f);
        SaveDebugImage(image_out_cv, "diffrenderhls_image_" + std::to_string(i) + ".png");
        SaveDebugImage(depth_out_cv, "diffrenderhls_depth_" + std::to_string(i) + ".png");
        SaveDebugImage(jtra_out_cv, "diffrenderhls_jtra_" + std::to_string(i) + ".png");
        SaveDebugImage(jrot_out_cv, "diffrenderhls_jrot_" + std::to_string(i) + ".png");
        SaveDebugImage(pids_out_cv, "diffrenderhls_pids_" + std::to_string(i) + ".png");
    }
